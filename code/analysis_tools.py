...
def pretty_label(col):
    ...
    # # pupil columns → always "Pupil diameter"
    # if base.startswith("mean_0_200") or base.startswith("mean_prev_m200_0") or base.startswith("mean_3042_3043"):
    #     return "Pupil diameter"

    # core behaviour names
    mapping = {
        "Correct":   "Hit rate",
        "Prec":      "Precision",
        "Bias":      "Bias (inv.)",
...
# (legacy rle removed — see rle() defined above with cleaner implementation)
...
#         session_data = session_data.iloc[:max_trials].copy()
...
#MISC
# (legacy plot_hidden_states removed — see updated version below with BoundaryNorm for consistent state colours)
    
    
# Fixes consistent state→color mapping across all subplots, legends, and imshow
# - Uses a fixed palette indexed strictly by state id: 0,1,2,3
# - Works even if only a subset of states appear (e.g., only 0 and 2)
# - Ensures imshow uses categorical boundaries (no normalization drift)
...
def plot_hidden_states(
    ...
    # ---- fixed state palette (index == state id) ----
    state_hex = ['#b96d85ff', '#4d9a80ff', '#37e6afff', '#fe89aeff']  # 0..3
    cmap_states = ListedColormap(state_hex, name='fixed_state_cmap')
    norm_states = BoundaryNorm(boundaries=np.arange(-0.5, 4.5, 1.0), ncolors=cmap_states.N)

    # ---- pull one session's data ----
    if isinstance(sesh_scores, list):
        sesh_df = sesh_scores[isesh]
    else:
        sesh_df = sesh_scores[sesh_scores['Session'] == isesh].reset_index(drop=True)

    hidden_states = sesh_df[states_column].to_numpy()
    present_states = np.unique(hidden_states).astype(int)  # actual states present in this session
    num_states_total = 4  # palette is for 0..3 (kept constant to preserve mapping)

    # ---- compute state means if not provided ----
    if state_means is None:
        # use ALL sessions, not the single session being plotted
        all_df = pd.concat(sesh_scores, ignore_index=True) if isinstance(sesh_scores, list) else sesh_scores
        states_all = all_df[states_column].to_numpy().astype(int)

        state_means = []
        for s in range(4):  # fixed palette indices 0..3
            mask = states_all == s
            if np.any(mask):
                state_means.append(np.nanmean(all_df.loc[mask, metrics].to_numpy(), axis=0))
            else:
                state_means.append(np.full(len(metrics), np.nan))

    # ---- metric colors (stable order by metrics list) ----
    cmap_metrics = plt.get_cmap("tab10")
    metric_colors = {m: cmap_metrics(i % 10) for i, m in enumerate(metrics)}
    ...
    fig.suptitle(f'Species {species}, states 0–3 (present: {",".join(map(str, present_states))}), session {isesh}')
    gs = gridspec.GridSpec(3, 1, height_ratios=[3, 1, 2])
    ax1 = plt.subplot(gs[0, :])
    ax2 = plt.subplot(gs[1, :])

    # top: metric traces with state overlay (categorical imshow with fixed norm)
    for m in metrics:
        ax1.plot(sesh_df[m].to_numpy(), label=m, color=metric_colors[m])
    ax1.set_xlim([0, limit])
    ax1.set_ylabel('Z-scored metrics')

    # categorical overlay: exact colors by state id (no interpolation normalization surprises)
    ax1.imshow(
        hidden_states[None, :],
        cmap=cmap_states,
        norm=norm_states,
        interpolation="none",
        aspect='auto',
        extent=(0, len(hidden_states), *ax1.get_ylim()),
        alpha=0.2
    )
    ax1.legend(loc='upper left', bbox_to_anchor=(1, 1))

    # middle: pure state stripe (same cmap+norm)
    ax2.imshow(
        hidden_states[None, :],
        cmap=cmap_states,
        norm=norm_states,
    ...
    # legend: strictly use color by state id (only for present states)
    patches = [mpatches.Patch(color=state_hex[s], label=STATE_NAMES.get(s, f'State {s}')) for s in present_states]
    ax2.legend(handles=patches, loc='upper left', bbox_to_anchor=(1, 1))

    # bottom: per-state bars using fixed color in titles (state color == palette[state])
    gs_bottom = gridspec.GridSpecFromSubplotSpec(1, num_states_total, subplot_spec=gs[2])
    axs = [plt.subplot(gs_bottom[i]) for i in range(num_states_total)]

    for s in range(num_states_total):
        vals = state_means[s]
        bars = axs[s].bar(metrics, vals, color=[metric_colors[m] for m in metrics])
        axs[s].tick_params(axis='both', which='major', labelsize=12)
        axs[s].set_title(STATE_NAMES.get(s, f'State {s}'), fontsize=15, color=state_hex[s])
        axs[s].set_xticklabels([])

    # put metric legend once
    axs[-1].legend(bars, metrics, loc='upper left', bbox_to_anchor=(1, 1))

    plt.tight_layout()


# =============================================================================
# SECTION: Analysis utilities (ported from analysis_pipeline.py inline defs)
# =============================================================================

def summarize(arr):
    """Percentile summary for a 1D array."""
    q1, med, q3 = np.percentile(arr, [25, 50, 75])
    return f"n={arr.size}, mean={np.mean(arr):.2f}, median={med:.3g}, IQR=[{q1:.3g}, {q3:.3g}]"
...
def collect_windows_for_transition(df, metric, window, from_state, to_state):
    ...
    for sesh_id, sesh in df.groupby('Session'):
        ...
        if sd == 0 or np.isnan(sd):
            zdiff = np.zeros_like(folded, dtype=float)
        else:
            zdiff = (folded - mu) / sd
...
def get_peak_positions(sess_means, x, window):
    """
    For each session spectrum, find the strongest peak within [window[0], window[1]].
    Returns: 1D array of peak positions (NaN if no peak in window).
    """
    ...
    for s in sess_means:
        ...
        if len(pks) == 0:
            ...
            continue
        pk_idx = pks[np.argmax(sw[pks])]
        peak_positions.append(xw[pk_idx])
    return np.array(peak_positions, dtype=float)


def analyze_cluster_reliability(corr_matrix, n_clusters=2, verbose=True):
    """
    Computes silhouette scores and identifies cluster membership for a correlation matrix.
    Per-metric silhouette breakdown: a = mean intra-cluster distance,
    b = mean nearest-other-cluster distance, s = (b - a) / max(a, b).
    """
...
# =============================================================================
# SECTION: Transition matrix statistics
# =============================================================================

def reassign_states_simple(states):
    """Relabel composite states 0-3 by standard quadrant ordering."""
    m = np.array([0, 3, 1, 2], dtype=int)  # old -> new: 0->0, 1->3, 2->1, 3->2
    s = np.asarray(states, dtype=int)
    return m[s]


def within_species_perm_test_two_streams(
    states_1,
    states_2,
    n_perm=2000,
    window=1,
    ...
    """
    Block-shuffle states_1 and states_2 separately, recombine as composite,
    and test transition matrices against null distribution.
    """
    rng = np.random.default_rng(random_state)
    states_1 = np.asarray(states_1).astype(int)
    states_2 = np.asarray(states_2).astype(int)
    assert states_1.shape == states_2.shape

    composite_states = states_1 + 2 * states_2
    S = int(np.max(composite_states)) + 1
    composite_states = reassign_states_simple(composite_states)
    P_obs = transition_prob_matrix(composite_states, num_states=S, window=window, minus_diag=minus_diag)

    null_stack = np.zeros((n_perm, S, S), dtype=float)
    for b in range(n_perm):
        shuf_1 = shuffle_blocks(states_1, rng=rng).astype(int)
        shuf_2 = shuffle_blocks(states_2, rng=rng).astype(int)
        shuf_comp = shuf_1 + 2 * shuf_2
        shuf_comp = reassign_states_simple(shuf_comp)
        null_stack[b] = transition_prob_matrix(shuf_comp, num_states=S, window=window, minus_diag=minus_diag)

    P_null_mean = null_stack.mean(axis=0)
    deltaP = P_obs - P_null_mean

    p_mat = np.ones((S, S), dtype=float)
    for i in range(S):
        for j in range(S):
            null_vals = null_stack[:, i, j]
            mu = P_null_mean[i, j]
            p_mat[i, j] = (np.sum(np.abs(null_vals - mu) >= abs(P_obs[i, j] - mu)) + 1) / (n_perm + 1)

    mask = ~np.eye(S, dtype=bool) if minus_diag else np.ones((S, S), dtype=bool)
    p_flat = p_mat[mask]
    _, q_flat, _, _ = multipletests(p_flat, alpha=alpha, method="fdr_bh")
    q_mat = np.full((S, S), np.nan)
    q_mat[mask] = q_flat
    sig_mask = np.zeros((S, S), dtype=bool)
    sig_mask[mask] = q_mat[mask] < alpha
    ...
    v_abs = np.nanmax(np.abs(deltaP)) if vmax_delta is None else vmax_delta
    fig, axes = plt.subplots(1, 3, figsize=(16, 5))

    sns.heatmap(P_obs, ax=axes[0], cmap="Blues", vmin=0, vmax=vmax_prob,
                annot=np.round(P_obs, 2), fmt="", cbar=True)
    axes[0].set_title("Observed transition P")
    axes[0].set_xlabel("To state"); axes[0].set_ylabel("From state")

    sns.heatmap(P_null_mean, ax=axes[1], cmap="Blues", vmin=0, vmax=vmax_prob,
                annot=np.round(P_null_mean, 2), fmt="", cbar=True)
    axes[1].set_title("Mean null (block-shuffled 2 streams)")
    axes[1].set_xlabel("To state"); axes[1].set_ylabel("")

    annot_delta = np.empty((S, S), dtype=object)
    for i in range(S):
        for j in range(S):
            val = deltaP[i, j]
            annot_delta[i, j] = "" if np.isnan(val) else (f"{val:.2f}" + ("*" if sig_mask[i, j] else ""))

    sns.heatmap(deltaP, ax=axes[2], cmap="RdBu_r", center=0, vmin=-v_abs, vmax=v_abs,
                annot=annot_delta, fmt="", cbar=True)
    axes[2].set_title("deltaP = P_obs - mean(null)\n* FDR<0.05")
    axes[2].set_xlabel("To state"); axes[2].set_ylabel("")

    plt.tight_layout()
    plt.show()

    print("\ndeltaP = P_obs - mean(null)  (* = FDR<0.05)")
    dp = np.round(deltaP, 3)
    for i in range(S):
        row = []
        for j in range(S):
            if minus_diag and i == j:
                row.append("   .   ")
            else:
                s_str = f"{dp[i, j]:+0.3f}"
                if sig_mask[i, j]:
                    s_str += "*"
                row.append(s_str.rjust(8))
        print(" ".join(row))

    return {
        "P_obs": P_obs, "P_null_mean": P_null_mean, "deltaP": deltaP,
        "p_values": p_mat, "q_values": q_mat, "sig_mask": sig_mask, "null_stack": null_stack,
    }
...
def format_annot(mat, decimals=2):
    """Format a matrix as an object array of strings for seaborn heatmap annotations."""
    out = np.empty(mat.shape, dtype=object)
    it = np.nditer(mat, flags=['multi_index'])
    while not it.finished:
        v = it[0].item()
        out[it.multi_index] = (f"{v:.{decimals}f}" if np.isfinite(v) else "")
        it.iternext()
    return out


def compare_species_transitions(
    species_loader,
    species_A,
    species_B,
    B_perm=2000,
    ...
    """
    Between-species permutation test + bootstrap confidence intervals for transition matrices.
    species_loader: callable(species_name) -> DataFrame with 'States' and 'Session' columns.
    """
    rng = default_rng(random_state)
    dfA = species_loader(species_A)
    dfB = species_loader(species_B)
    S = int(max(dfA['States'].max(), dfB['States'].max())) + 1

    A_counts_list = aggregate_species_counts(dfA, num_states=S, window=window, session_weighting=False)
    B_counts_list = aggregate_species_counts(dfB, num_states=S, window=window, session_weighting=False)
    nA, nB = len(A_counts_list), len(B_counts_list)

    C_A = np.sum(A_counts_list, axis=0)
    C_B = np.sum(B_counts_list, axis=0)
    P_A = normalize_rows(C_A, minus_diag=minus_diag)
    P_B = normalize_rows(C_B, minus_diag=minus_diag)

    diff_obs = P_A - P_B
    T_obs = np.linalg.norm(diff_obs, ord='fro')
    offdiag_mask = ~np.eye(S, dtype=bool) if minus_diag else np.ones((S, S), dtype=bool)

    all_counts = A_counts_list + B_counts_list
    idx_all = np.arange(len(all_counts))
    T_null = np.empty(B_perm, dtype=float)
    diff_null = np.zeros((B_perm, S, S), dtype=float)

    for b in range(B_perm):
        perm = rng.permutation(idx_all)
        A_idx, B_idx = perm[:nA], perm[nA:]
        C_A_b = np.sum([all_counts[i] for i in A_idx], axis=0)
        C_B_b = np.sum([all_counts[i] for i in B_idx], axis=0)
        P_A_b = normalize_rows(C_A_b, minus_diag=minus_diag)
        P_B_b = normalize_rows(C_B_b, minus_diag=minus_diag)
        D_b = P_A_b - P_B_b
        diff_null[b] = D_b
        T_null[b] = np.linalg.norm(D_b, ord='fro')

    p_global = (1 + np.sum(T_null >= T_obs)) / (B_perm + 1)

    p_matrix = np.ones((S, S))
    for i in range(S):
        for j in range(S):
            if not offdiag_mask[i, j]:
                p_matrix[i, j] = np.nan
                continue
            null_vals = diff_null[:, i, j]
            obs_val = diff_obs[i, j]
            p_matrix[i, j] = (np.sum(np.abs(null_vals) >= abs(obs_val)) + 1) / (B_perm + 1)

    tested = np.where(offdiag_mask.ravel())[0]
    p_flat = p_matrix.ravel()
    _, q_flat, _, _ = multipletests(p_flat[tested], alpha=0.05, method='fdr_bh')
    q_matrix = np.full_like(p_matrix, np.nan, dtype=float)
    q_matrix.ravel()[tested] = q_flat
    sig_mask = np.zeros_like(offdiag_mask, dtype=bool)
    sig_mask.ravel()[tested] = q_flat < 0.05

    boot_diffs = np.zeros((B_boot, S, S))
    for b in range(B_boot):
        A_idx = rng.integers(0, nA, size=nA)
        B_idx = rng.integers(0, nB, size=nB)
        C_A_b = np.sum([A_counts_list[i] for i in A_idx], axis=0)
        C_B_b = np.sum([B_counts_list[i] for i in B_idx], axis=0)
        P_A_b = normalize_rows(C_A_b, minus_diag=minus_diag)
        P_B_b = normalize_rows(C_B_b, minus_diag=minus_diag)
        boot_diffs[b] = P_A_b - P_B_b

    ci_low = np.percentile(boot_diffs, 2.5, axis=0)
    ci_high = np.percentile(boot_diffs, 97.5, axis=0)

    if minus_diag:
        for M in (diff_obs, p_matrix, q_matrix, ci_low, ci_high):
            np.fill_diagonal(M, np.nan)

    deltaP = diff_obs.copy()
    H = cohen_h(P_A, P_B)
    if minus_diag:
        np.fill_diagonal(H, np.nan)

    print(f"Between-species global permutation test ({species_A} vs {species_B})")
    print(f"  Frobenius distance (observed): {T_obs:.6f}")
    print(f"  Permutation p-value (B={B_perm}): {p_global:.6g}")

    v_abs = np.nanmax(np.abs(deltaP)) if np.isfinite(deltaP).any() else 1.0
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))

    sns.heatmap(P_A, cmap="Blues", vmin=0, vmax=1,
                annot=format_annot(P_A, annot_decimals), fmt="", ax=axes[0])
    axes[0].set_title(f"{species_A}: P (row-normalized)")
    axes[0].set_xlabel("To state"); axes[0].set_ylabel("From state")

    sns.heatmap(P_B, cmap="Blues", vmin=0, vmax=1,
                annot=format_annot(P_B, annot_decimals), fmt="", ax=axes[1])
    axes[1].set_title(f"{species_B}: P (row-normalized)")
    axes[1].set_xlabel("To state"); axes[1].set_ylabel("")

    annot_ci = np.empty(deltaP.shape, dtype=object)
    for i in range(S):
        for j in range(S):
            if np.isnan(deltaP[i, j]):
                annot_ci[i, j] = ""
                continue
            lo, hi = ci_low[i, j], ci_high[i, j]
            annot_ci[i, j] = f"{deltaP[i,j]:.2f}\n[{lo:.2f},{hi:.2f}]"

    sns.heatmap(deltaP, cmap="RdBu_r", center=0, vmin=-v_abs, vmax=v_abs,
                annot=annot_ci, fmt="", ax=axes[2], cbar=True)
    axes[2].set_title(f"deltaP = {species_A} - {species_B}\n95% CIs (bootstrapped)\n* FDR<0.05")
    axes[2].set_xlabel("To state"); axes[2].set_ylabel("")

    for i in range(S):
        for j in range(S):
            if sig_mask[i, j]:
                axes[2].text(j + 0.7, i + 0.3, '*', ha='center', va='center',
    ...
    return {
        "P_A": P_A, "P_B": P_B, "deltaP": deltaP, "cohen_h": H,
        "p_matrix": p_matrix, "q_matrix": q_matrix, "sig_mask": sig_mask,
        "ci_low": ci_low, "ci_high": ci_high, "T_obs": T_obs, "p_global": p_global
    }


# =============================================================================
# SECTION: Speed-accuracy coupling analysis (for Suppl Fig 7C)
# =============================================================================

def _switch_series(x):
    """Binary series of 1 where state switches occur."""
    x = np.asarray(x, int)
    sw = np.zeros_like(x, dtype=int)
    sw[1:] = (x[1:] != x[:-1]).astype(int)
    return sw


def coupling_C_lagcurve(speed_states, acc_states, max_lag=10):
    """
    C(l) = P(acc switch at t+l | speed switch at t), for l in [-max_lag..+max_lag].
    Returns lags, C(l), n_events.
    """
    speed_sw = _switch_series(speed_states)
    acc_sw   = _switch_series(acc_states)
    t_events = np.where(speed_sw == 1)[0]
    n = len(speed_sw)
    lags = np.arange(-max_lag, max_lag + 1)
    C = np.full(len(lags), np.nan, dtype=float)
    for k, lag in enumerate(lags):
        tt = t_events + lag
        valid = (tt >= 0) & (tt < n)
        if valid.sum() == 0:
            continue
        C[k] = acc_sw[tt[valid]].mean()
    return lags, C, len(t_events)


def _circular_shift_in_groups(x, groups, rng, min_shift=1):
    """Circularly shift x within each group (session) by a random offset."""
    x = np.asarray(x)
    ...
    for g in np.unique(groups):
        ...
        if n <= 1:
            out[idx] = seg
            continue
        lo = min_shift if n > min_shift else 1
        shift = rng.integers(lo, n)
        out[idx] = np.roll(seg, shift)
    return out


def coupling_perm_test_circshift(
    speed_states, acc_states,
    sessions,
    n_perm=2000, max_lag=5, random_state=0,
    alpha=0.05, summary_lags=(1, 2, 3),
    min_shift=5
):
    """
    Test speed-accuracy switch coupling using circular-shift null within sessions.
    sessions: array of session labels aligned to speed_states/acc_states.
    """
    rng = np.random.default_rng(random_state)
    speed_states = np.asarray(speed_states, int)
    acc_states   = np.asarray(acc_states, int)
    sessions     = np.asarray(sessions)
    assert speed_states.shape == acc_states.shape == sessions.shape

    lags, C_obs, n_events = coupling_C_lagcurve(speed_states, acc_states, max_lag=max_lag)

    null_C = np.zeros((n_perm, len(lags)), dtype=float)
    for b in range(n_perm):
        acc_sh = _circular_shift_in_groups(acc_states, sessions, rng=rng, min_shift=min_shift).astype(int)
        _, Cb, _ = coupling_C_lagcurve(speed_states, acc_sh, max_lag=max_lag)
        null_C[b] = Cb
    ...
    p = np.ones(len(lags), float)
    for k in range(len(lags)):
        mu = null_mean[k]
        p[k] = (np.sum(np.abs(null_C[:, k] - mu) >= abs(C_obs[k] - mu)) + 1) / (n_perm + 1)

    _, q, _, _ = multipletests(p, alpha=alpha, method="fdr_bh")
    sig = q < alpha

    summary_lags_arr = np.array(list(summary_lags))
    idx = np.isin(lags, summary_lags_arr)
    summary_obs  = np.nanmean(C_obs[idx])
    summary_null = np.nanmean(null_C[:, idx], axis=1)
    mu_sum = np.nanmean(summary_null)
    p_sum = (np.sum(summary_null >= summary_obs) + 1) / (n_perm + 1)

    return {
        "lags": lags, "C_obs": C_obs, "null_C": null_C,
        "null_mean": null_mean, "null_lo": null_lo, "null_hi": null_hi,
        "p_lag": p, "q_lag": q, "sig_lag": sig,
        "n_speed_switches": n_events, "summary_lags": summary_lags,
        "summary_obs": summary_obs, "summary_null_mean": mu_sum, "summary_p": p_sum,
        "min_shift": min_shift,
    }


def plot_coupling(res, title, species):
    """Plot lagged coupling C(l) with null CI and significance markers."""
    lags  = res["lags"]
    Cobs  = res["C_obs"]
    Cnull = res["null_mean"]
    sig   = np.asarray(res.get("sig_lag", np.zeros_like(lags, dtype=bool)), dtype=bool)

    fig, ax = plt.subplots(figsize=(4, 3.2), dpi=150)
    ax.plot(lags, Cnull, marker="o", markersize=3.5, linewidth=1.8,
            color="#6A5ACD", label="Null mean (circ-shift)")
    ax.fill_between(lags, res["null_lo"], res["null_hi"], alpha=0.25,
                    color="#6A5ACD", label="Null 95% CI")
    ax.plot(lags, Cobs, marker="o", markersize=3.5, linewidth=2.2,
            color="#2F80ED", label="Observed")
    if sig.any():
        ax.scatter(lags[sig], Cobs[sig], color="#D81B60", s=28, zorder=5, label="FDR < 0.05")

    ax.axvline(0, linewidth=1, color="0.3", alpha=0.7)
    ax.set_xticks(lags)
    ax.set_xlabel("Lag l (trials)")
    ax.set_ylabel("P(accuracy switch at t+l | speed switch at t)")
    ax.set_title(title)
    ...
    fig.tight_layout()
    save_plot(f"coupling_circshift_{species}.png")
    plt.show()


# =============================================================================
# SECTION: Spectral analysis helpers (for Figs 4, S8)
# =============================================================================

def _repeat_to_time(y, dur_s, fs_hz):
    """Step-hold trials into a uniform time grid. Ensures >=1 sample per trial."""
    counts = np.maximum(1, np.round(np.asarray(dur_s, float) * fs_hz).astype(int))
    return np.repeat(np.asarray(y, float), counts)


def superlets_per_session_seconds(df, metric, fs_hz, foi_hz, c1=None, ord=None):
    """Return list of [n_freq x n_time] power arrays (one per session) in seconds domain."""
    power_per_sesh = []
    for sesh, g in df.groupby('Session', sort=True):
        y   = g[metric].to_numpy()
        dur = g['TrialDuration'].to_numpy()
        y_uni = _repeat_to_time(y, dur, fs_hz)
        if c1 is None or ord is None:
            P = slt.superlets(y_uni[np.newaxis, :], fs_hz, foi_hz)
        else:
            P = slt.superlets(y_uni[np.newaxis, :], fs_hz, foi_hz, c1, ord)
        power_per_sesh.append(P)
    return power_per_sesh


def session_mean_spectrum(power_list):
    """Compute mean power spectrum per session (averaged over time)."""
    sess_means = []
    for P in power_list:
        P = np.squeeze(P)
        if P.ndim == 2:
            sess_means.append(P.mean(axis=1))
    if sess_means:
        return np.vstack(sess_means)
    return np.empty((0, 0))


# =============================================================================
# SECTION: Pupil analysis utilities (for Suppl Fig 10)
# =============================================================================
